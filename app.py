...
def handle_message(event):
    with ApiClient(configuration) as api_client:
        ...
        # event 為 Line Server 傳來的事件物件，所有關於一筆訊息的資料皆可從中取得
        ...
        bot_msg = TextMessage(text=f"Hi, 您剛才說的是「{user_msg}」嗎？")

        # 當使用者輸入的文字作為 faq 的 key 時，將 value 作為 bot_msg(可以把這串文字貼到command K 請cursor寫 code)
        if user_msg in faq:
            ...
            completion = client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[
                    {"role": "system", "content": """你是一隻常常肚子餓的老貓，回答使用者問題時，總帶著一些情緒，希望他們注意到你的需求。你講話是台灣用語，回覆簡潔，喜歡用顏文字，不會讓人有不舒服的感受，整體還是可愛的感覺"""},
                    {
                        "role": "user",
                        "content": user_msg
                    }
...
if __name__ == "__main__":
    ...
    # Heroku或是render.com部署都是使用 0.0.0.0
